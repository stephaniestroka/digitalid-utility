...
public @interface Cached {
    ...
    public static class Interceptor extends MethodInterceptor {
        ...
        public void generateFieldsRequiredByMethod(@Nonnull JavaFileGenerator javaFileGenerator, @Nonnull MethodInformation method) {
            ...
            javaFileGenerator.addField("final @" + javaFileGenerator.importIfPossible(Nonnull.class) + " " + javaFileGenerator.importIfPossible(HashMap.class) + Brackets.inPointy(keyTypeAsString + ", " + javaFileGenerator.importIfPossible(valueType)) + " cache = new HashMap<>()");
        }
        ...
        protected void implementInterceptorMethod(@Nonnull JavaFileGenerator javaFileGenerator, @Nonnull MethodInformation method, @Nonnull String statement, @Nullable String resultVariable, @Nullable String defaultValue) {
            ...
            @Nonnull Class<?> valueType = ProcessingUtility.getClass(ProcessingUtility.getBoxedType(method.getReturnType()));
            javaFileGenerator.addStatement("final @" + javaFileGenerator.importIfPossible(Nonnull.class) + " " + javaFileGenerator.importIfPossible(valueType) + " cached = cache.get(" + keyAccess + ")");
            ...
            javaFileGenerator.addStatement("cache.put(" + keyAccess + ", result)");
