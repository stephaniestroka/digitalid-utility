...
@Validator(LessThanOrEqualTo.Validator.class)
    ...
    public static class Validator extends AnnotationValidator {
        ...
        public @Nonnull GeneratedContract generateContract(@Nonnull Element element, @Nonnull AnnotationMirror annotationMirror, @Nonnull TypeImporter typeImporter) {
            if (ProcessingUtility.isAssignable(element, BigInteger.class)) {
                return GeneratedContract.with("# == null || #.compareTo(" + typeImporter.importIfPossible(BigInteger.class) + ".valueOf(@)) <= 0", "The # has to be null or less than or equal to @ but was $.", element, annotationMirror);
            } else {
                return GeneratedContract.with("# <= @", "The # has to be less than or equal to @ but was $.", element, annotationMirror);
            }
        }
