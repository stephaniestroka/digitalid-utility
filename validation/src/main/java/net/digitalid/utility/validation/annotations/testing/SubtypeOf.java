...
/**
 * This annotation indicates that the annotated type is a subtype of the given type.
 * 
 * @see NotSubtypeOf
 */
@Target(ElementType.TYPE_USE)
@Retention(RetentionPolicy.RUNTIME)
@ValueValidator(SubtypeOf.Validator.class)
public @interface SubtypeOf {
    
    /* -------------------------------------------------- Value -------------------------------------------------- */
    
    /**
     * Returns the type of which the annotated type should be a subtype.
     */
    @Nonnull Class<?> value();
    ...
    public static class Validator implements ValueAnnotationValidator {
        ...
        public void checkUsage(@Nonnull Element element, @Nonnull AnnotationMirror annotationMirror, @NonCaptured @Modified @Nonnull ErrorLogger errorLogger) {
            ...
            if (desiredType != null && !ProcessingUtility.isSubtype(declaredType, desiredType)) {
                errorLogger.log("The type $ is not a subtype of $:", SourcePosition.of(element), ProcessingUtility.getSimpleName(declaredType), desiredType.getSimpleName());
            }
        }
