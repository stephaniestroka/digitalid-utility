...
@Validator(NonPositive.Validator.class)
    ...
    public static class Validator extends AnnotationValidator {
        ...
        public @Nonnull GeneratedContract generateContract(@Nonnull Element element, @Nonnull AnnotationMirror annotationMirror, @Nonnull TypeImporter typeImporter) {
            if (ProcessingUtility.isAssignable(element, BigInteger.class)) {
                return GeneratedContract.with("# == null || #.compareTo(" + typeImporter.importIfPossible(BigInteger.class) + ".ZERO) <= 0", "The # has to be null or non-positive but was $.", element);
            } else {
                return GeneratedContract.with("# <= 0", "The # has to be non-positive but was $.", element);
            }
        }
